...
Perform a structured security audit of the codebase following the OWASP Top 10 methodology, with platform-specific checks based on the detected technology stack. The audit covers: attack surface mapping, secrets scanning, dependency analysis, systematic vulnerability review, and severity classification.
...
5. **Scan dependencies**: Follow Step 3 of the `security-auditing` skill. Run the appropriate dependency audit command for each project in scope and cross-reference findings with known CVEs.
6. **Systematic vulnerability review**: Follow Steps 4-5 of the `security-auditing` skill. Go through each OWASP Top 10 category and platform-specific checks based on the detected tech stack (mobile, backend, database, infrastructure, frontend). Skip categories not applicable to the stack.
