...
- `architecture-designing` - for the architecture design process and output template (`plan.example.md`)
- `codebase-analysing` - for analyzing the existing codebase
- `implementation-gap-analysing` - for verifying what exists vs what needs to be built
- `technical-context-discovering` - for understanding project conventions and patterns
- `sql-and-database-understanding` - when the feature involves database schema design, data model changes, migrations, or query-heavy implementation
...
9. **Save the plan**: Follow the `plan.example.md` template from the `architecture-designing` skill strictly.
