...
3. Focus only on implementation plan, don't try to implement anything that is not part of the plan unless explicitly instructed.
4. Don't implement improvements from the plans improvements section unless explicitly instructed.
5. Start implementing the feature according to the plan, following each task step by step.
6. After completing each task step, update the relevant plan to reflect the progress made by checking the box for the completed task step in the plan document.
7. After each phase make sure to:
...
8. Before making any changes to the original solution during implementation ask for confirmation. Make sure to document those changes in the specified plan file in Changelog section.
9. At the end of the implementation, perform a final review of the entire feature following acceptance criteria checklist to ensure all tasks have been completed as per the plan and that the feature meets the defined requirements. Make sure to update the acceptance criteria checklist after every verified item.
