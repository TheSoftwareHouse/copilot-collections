...
- **Error handling wrappers**: Wrap data-dependent sections with error handling boundaries (e.g., React error boundaries, Vue `errorCaptured`, Angular `ErrorHandler`) for graceful failure.
...
- Use the `ui-verifying` skill to compare the implementation against the Figma design.
...
- `ui-verifying` — to verify implementation matches Figma design after building components
- `ensuring-accessibility` — to ensure components meet WCAG 2.1 AA standards
- `optimizing-frontend` — for performance considerations during component implementation
- `implementing-forms` — for form-specific component patterns and validation
- `writing-hooks` — for custom hook patterns used within components
- `reviewing-frontend` — for frontend-specific code review of implemented components
- `technical-context-discovering` — to understand project conventions before implementing
