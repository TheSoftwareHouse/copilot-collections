...
6. **Handle `[REUSE]` UI verification as a per-item loop:**
   ...
   - Delegate each item to `tsh-ui-reviewer` with `tsh-review-ui.prompt.md`, passing the Figma URL, the pinned user-confirmed full dev server URL unchanged, and the component or section name.
